#!/usr/bin/env bash
#
# Enforces coding-standards naming conventions for the FastAPI/Python backend.
#
# Checks:
#   1. File names: snake_case.py (except __init__.py, conftest.py, test_*.py)
#   2. Class names: PascalCase (e.g. VesselPoint, UserApiSchema)
#   3. Function names: snake_case (e.g. fetch_trajectory, get_vessel_playback)
#   4. Method names: snake_case
#   5. Module-level variables: snake_case or UPPER_SNAKE_CASE (no camelCase)
#   6. Local variables: snake_case (no camelCase)
#   7. Test files: must start with test_ prefix
#
# Usage: ./check-backend-naming-conventions.sh <src_dir>
#
set -euo pipefail
...
echo "=========================================="
echo "  Backend Naming Convention Check"
...
while IFS= read -r py_file; do
  ...
  if ! is_snake_case "$stem"; then
    ...
    echo "        Expected: snake_case.py (e.g. vessel_utils.py), got: '$filename'"
    ERRORS=$((ERRORS + 1))
  fi
...
while IFS= read -r py_file; do
  ...
  while IFS= read -r class_name; do
    ...
    if ! is_pascal_case "$class_name"; then
      ...
      echo "        Expected: PascalCase (e.g. VesselPoint, UserApiSchema)"
      ERRORS=$((ERRORS + 1))
    fi
...
while IFS= read -r py_file; do
  ...
  while IFS= read -r func_name; do
    ...
    if ! is_snake_case "$func_name"; then
      ...
      echo "        Expected: snake_case (e.g. fetch_trajectory, get_vessel_playback)"
      ERRORS=$((ERRORS + 1))
    fi
...
echo ""
echo "-- Check 4: Method names must be snake_case (indented def) --"
...
# ── Check 5: Module-level variables — no camelCase ───────────────────────
echo ""
...
while IFS= read -r py_file; do
  ...
  while IFS= read -r line; do
    ...
    if ! is_snake_case "$var_name" && ! is_upper_snake_case "$var_name"; then
      ...
      echo "        camelCase/PascalCase variables are not allowed at module level in Python"
      ERRORS=$((ERRORS + 1))
    fi
...
while IFS= read -r py_file; do
  ...
  while IFS= read -r line; do
    ...
    if is_camel_case "$var_name"; then
      ...
      echo "        Python variables must use snake_case (e.g. vessel_id, not vesselId)"
      ERRORS=$((ERRORS + 1))
    fi
...
# ── Check 7: Test files must start with test_ prefix ─────────────────────
echo ""
...
while IFS= read -r test_file; do
  ...
  if [[ "$filename" != test_*.py ]]; then
    ...
    echo "        Expected: test_foo.py, got: '$filename'"
    ERRORS=$((ERRORS + 1))
  fi
...
if [ "$ERRORS" -gt 0 ]; then
  ...
  echo "  1. File names: snake_case.py (e.g. vessel_utils.py)"
  echo "  2. Class names: PascalCase (e.g. VesselPoint, UserApiSchema)"
  echo "  3. Function names: snake_case (e.g. fetch_trajectory, get_vessel_playback)"
  echo "  4. Method names: snake_case (e.g. get_name, set_value)"
  echo "  5. Module-level variables: snake_case or UPPER_SNAKE_CASE"
  echo "  6. Local variables: snake_case (no camelCase)"
  echo "  7. Test files: must start with 'test_' prefix (e.g. test_vessels.py)"
  exit 1
else
